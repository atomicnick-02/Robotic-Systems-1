...
class EKF_SLAM:
	...
	def __init__(self):
		...
		# flag to indicate if we are using KL divergence for correction
		self.use_kl_divergence = True
		# flag to indicate if we are merging close landmarks
		self.merge_close_landmarks = True
		...
		for hyp in self.hypotheses:
			for zi in z:
				...
				if not associations:
					...
					for j, dz, H, K, _ in associations:
						mu_new = hyp.mu + K @ dz
						mu_new[2] = self.wrap_angle(mu_new[2])
						Sigma_new = (np.eye(len(mu_new)) - K @ H) @ hyp.Sigma

						# Compute KL divergence from old to new Gaussian
						try:
							inv_Sigma = np.linalg.inv(Sigma_new)
							d_mu = mu_new - hyp.mu
							kl = 0.5 * (np.trace(inv_Sigma @ hyp.Sigma) +
										d_mu.T @ inv_Sigma @ d_mu -
										len(mu_new) +
										np.log(np.linalg.det(Sigma_new) / np.linalg.det(hyp.Sigma)))
						except np.linalg.LinAlgError:
							kl = np.inf  # fallback if Sigma_new is singular
